...
Several reasonable identity and credential fixes failed. Instead of repeating them indefinitely, the investigation moved **down the stack** toward the biometric layer itself.

The final fix was unexpected: enabling the Windows setting that allows Windows Hello to use an **external camera or fingerprint sensor**. No external device was ever connected. Flipping the toggle and retrying the built-in webcam was the entire process: the toggle re-initialized the biometric stack, and Face enrollment completed **on the camera that had been failing all along**. The toggle's documented purpose had nothing to do with the problem. What mattered was the side effect.
...
## Layer 5: The hardware test that never happened

The next planned step was introducing alternate biometric hardware (an external camera or fingerprint reader) to establish whether the Hello stack could complete an enrollment at all on this machine.

Step one of that plan was enabling the Windows setting that permits external Hello devices. Before any external device was connected, the built-in webcam was retried.

**Enrollment succeeded.** The hardware test was never needed.
...
After that toggle was enabled, Windows Hello Face **enrollment completed on the built-in webcam**, the same sensor that had failed every previous attempt. No external camera or fingerprint reader was ever attached. The toggle exists to control external biometric devices, yet flipping it fixed the internal one.
